...
public class GameLeaderboard {
     ...
     *
     * @param gameName The short name of the game
     * @throws Dota2Exception if an error occurs while fetching the
     *         leaderboards
     */
    private static void loadLeaderboards(String gameName)
        ...
        try {
            ...
            if(boardsData.hasElement("error")) {
                throw new Dota2Exception(boardsData.getString("error"));
            }
    ...
    public GameLeaderboardEntry getEntryForSteamId(long steamId)
        ...
        try {
            ...
            if(boardData.hasElement("error")) {
                throw new Dota2Exception(boardData.getString("error"));
            }
    ...
    public Map<Integer, GameLeaderboardEntry> getEntryRange(int first, int last)
            throws Dota2Exception {
        if(last < first) {
            throw new Dota2Exception("First entry must be prior to last entry for leaderboard entry lookup.");
        }
        if((last - first) > 5000) {
            throw new Dota2Exception("Leaderboard entry lookup is currently limited to a maximum of 5001 entries per request.");
        }
